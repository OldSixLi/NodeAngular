  ...
  return new Promise(function(resolve, reject) {
    nodegrass.get(playUrl, data => {
      ...
      if (musicArr.length > 0) {
        ...
        let musicObjArr = [];
        musicArr.forEach((musicObj, index) => {
          musicObjArr.push([musicObj.id,
            musicNameArr[index] && musicNameArr[index].name || "", musicNameArr[index] && musicNameArr[index].ar && musicNameArr[index].ar[0] && musicNameArr[index].ar[0].name || "", 0, playId, getNowFormatDate()
          ])
        });
